...
const file = data.image[0];
  ...
  return (
    <div className= {`fixed top-0 left-0 z-50 w-full h-dvh ${addProduct? 'opacity-100 visible':'opacity-0 invisible'} transition-all duration-300 bg-black/70 overflow-hidden`} onClick={()=>setAddProduct(false)}>
        <div className={`bg-sidebar h-full lg:w-3/5 max-lg:w-[90%] ${addProduct? 'translate-x-0':'translate-x-full'} transition-all duration-400 absolute top-0 right-0`} onClick={(e)=> e.stopPropagation()}>

        <div className="title flex items-center justify-between lg:p-7 p-3 border-b-2">
            <div className='font-bold '>
                <h1 className='text-xl'>Add Product</h1>
                <p className='text-sm text-foreground/60'>Add necessary product information here</p>
            </div>
            <X className='size-7 cursor-pointer' onClick={()=>setAddProduct(false)}/>
        </div>
        ...
        <div className="content overflow-y-auto lg:h-[76%] max-lg:h-[78%] mb-3 w-full bg-muted lg:p-7 p-4">
            <form onSubmit={handleSubmit(mutate)} className='flex flex-col  mb-3 lg:gap-10 gap-9'>
                ...
                <div className='lg:flex-1 max-lg:w-full lg:p-2'>
                <textarea type="text" placeholder='Product Description...' className='lg:flex-1 h-30 rounded-[9px] w-full max-lg:w-full p-2 border-1 border-foreground' id='description' {...register('description')} required/>
                {errors.description && <p className='pl-1 text-sm text-red-500 font-bold'>* {errors.description?.message}</p>}
                </div>
                </label>

                <label htmlFor="image" className='flex max-lg:flex-col lg:gap-15 gap-1 lg:items-center'>
                <span className='font-bold text-lg'>Product Image</span>    
                <div className='lg:flex-1 max-lg:w-full lg:p-2'>
                <input type="file" className='lg:flex-1 cursor-pointer w-full border-1 mb-1 border-foreground rounded-[8px] max-lg:w-full p-2 border-2' id='image' {...register('image')} required/>
                ...
                <label className='flex max-lg:flex-col lg:gap-15 gap-1 lg:items-center'>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        ...
                        <div className='lg:flex-1 max-lg:max-w-full lg:p-2'>
            ...
            <div className="buttons flex gap-2 px-4 w-full *:flex-1 *:p-4.5 *:text-lg font-bold">
                <Button className='bg-foreground/40 rounded-[8px] hover:bg-foreground/20 cursor-pointer'  onClick={()=>setAddProduct(false)}>Cancel</Button>
                <Button type="button" onClick={handleSubmit(mutate)} className='rounded-[8px] cursor-pointer' disabled={isPending}>{isPending? <><span>Adding</span> <Spinner/></>:'Add Product'}</Button>
            </div>
        </div>
    </div>
  )
